...
// DATA VALIDATION
const {error} = signUpValidation({username:req.body.username, email:req.body.email, password: req.body.password})
if (error) return res.status(400).json(error.details[0].message)

//checking if the email exists
const emaiExist = await User.findOne({email:req.body.email})
if(emaiExist) { res.status(400).json("Email already exists")}
...
app.post('/login', async (req,res)=>{
	// DATA VALIDATION
	const {error} = loginValidation({username:req.body.username, email:req.body.email, password: req.body.password})
	if (error) return res.status(400).json(error.details[0].message)
	//check email
